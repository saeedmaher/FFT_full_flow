module DIF_Butterfly_St3 #(
    ...
    assign sum_q57_re =
        add_re[OUT_WIDTH-1:0];

    assign sum_q57_im =
        add_im[OUT_WIDTH-1:0];


    assign diff_q57_re =
        sub_re[OUT_WIDTH-1:0];

    assign diff_q57_im =
        sub_im[OUT_WIDTH-1:0];
    ...
    assign mult_result_re =
        {mult_re_re[23], mult_re_re}
        -
        {mult_im_im[23], mult_im_im};


    assign mult_result_im =
        {mult_re_im[23], mult_re_im}
        +
        {mult_im_re[23], mult_im_re};
